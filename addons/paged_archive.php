...
if (isset($language_abr) && $language_abr == $default_language_abr)	$headline_selection = 'headline';
...
if ($table != "")
{
	...
	while($rows=mysqli_fetch_array($result)){$result_id[] = $rows;}
	//print_r($result_id);

	for($t = 0; $t < count($result_id); $t++)
	{
		if (strtolower( $fieldname) == strtolower($result_id[$t]['Field']))
		{
			$fieldexists = 1;
			break;
		}
...
if($fieldexists==0)
{
	$result = mysqli_query($con, "ALTER TABLE $table ADD `maxpthumb` VARCHAR( 5) DEFAULT '100' NOT NULL ");
}
...
$photonumb = mysqli_query($con, "SELECT count(*) AS count FROM ".$pixelpost_db_prefix."pixelpost WHERE datetime<='$datetime'");
$row = mysqli_fetch_array($photonumb);
...
if (isset($language_abr) && $language_abr == $default_language_abr)	$query = mysqli_query($con, "SELECT * FROM ".$pixelpost_db_prefix."categories ORDER BY name");
else	$query = mysqli_query($con, "SELECT * FROM ".$pixelpost_db_prefix."categories ORDER BY alt_name");

while(list($id, $name, $alt_name) = mysqli_fetch_row($query))
{
	...
	GROUP BY t2.id, t1.image_id, t1.cat_id";
	//echo $queryr;
...
if (isset($_GET['pagenum']))
{
	...
	// $pixelpost_all_photonumb = all photo numbers;
  ...
  //query used to be SELECT DISTINCT ... this is not working with higher version of MYSQL
	$query = mysqli_query($con, $query);
	while(null!=$query && $tmp[] = mysqli_fetch_row($query))
	$i = 0;
	while($i < count($tmp)-1)
	{
		$thedate = $tmp[$i][0];
...
else
{
	...
	$query = mysqli_query($con, "SELECT DATE_FORMAT(datetime, '%Y-%m') FROM ".$pixelpost_db_prefix."pixelpost ORDER BY datetime desc");

	while(null!=$query && list($thedate) = mysqli_fetch_row($query))
	{
		$query3 = "SELECT count(*) AS count FROM ".$pixelpost_db_prefix."pixelpost WHERE DATE_FORMAT(datetime, '%Y-%m')='".$thedate ."'";
		$count = mysqli_query($con, $query3);
		$count = mysqli_fetch_array($count);
		$count= $count['count'];
...
if(isset($_GET['x'])&&$_GET['x'] == "browse")
{
	...
	{	
  	if ($language_abr == $default_language_abr)	$tag_selection = "AND (t2.tag = '" . $x_tag . "')";
	  else	$tag_selection = "AND (t2.alt_tag = '" . $x_tag . "')";
		$query = "SELECT t1.id,t1.{$headline_selection},t1.image
	...
	// for each record ...
	while(list($id,$title,$name) = mysqli_fetch_row($query))
	{
		...
		$thumb_output .= "<a href='admin/$PHP_SELF?view=images&id=$id' target='_blank'><img src='".$thumbnail."' alt='".$title."' title='".$title."' width='".$local_width."' height='".$local_height."' class='thumbnails' /></a>";
	}
	} //end while
	...
	{
  	if ($language_abr == $default_language_abr)	$tag_selection = "AND (t2.tag = '" . mysqli_real_escape_string($con, $_GET['tag']) . "')";
	  else	$tag_selection = "AND (t2.alt_tag = '" . mysqli_real_escape_string($con, $_GET['tag']) . "')";
	...
	{
		$archivedate = preg_replace('/[^0-9\-]/', '', $_GET['archivedate']);
		$archivedate_start = mysqli_real_escape_string($con, $archivedate) ."-01 00:00:00";
		$archivedate_end = mysqli_real_escape_string($con, $archivedate) ."-31 23:59:59";
		$where = " AND datetime >='$archivedate_start' AND datetime <= '$archivedate_end' ";
		$monthname = mysqli_real_escape_string($con, $_GET['monthname']);
	...
	$photonumb = mysqli_query($con, $queryr);
	if (null!=$photonumb) $row = mysqli_fetch_array($photonumb);
	...
	else if($paged_arch_tag_flag > 0)
	{
		...
		while ($pagecounter < $num_browse_pages)
		{
			$pagecounter++;
			if ($pagecounter != $pagenum){
				$Archive_pages_Links .= "<span class=\"archive_page_number\"><a href='index.php?x=browse&amp;tag=".mysqli_real_escape_string($con, $_GET['tag'])."&amp;pagenum=$pagecounter'>$pagecounter</a></span> ";
			} else {
				$Archive_pages_Links .= "<span class=\"archive_active_page_number\"><a href='index.php?x=browse&amp;tag=".mysqli_real_escape_string($con, $_GET['tag'])."&amp;pagenum=$pagecounter'>$pagecounter</a></span> ";
	...
	else if($paged_arch_archdate_flag > 0)
	{
		...
		while ($pagecounter < $num_browse_pages)
		{
			$monthname=str_replace(" ","%20",$monthname);
			$pagecounter++;
			$archivedate=mysqli_real_escape_string($con, $_GET['archivedate']);
			if ($pagecounter != $pagenum){
	...
	if($cat_id != "")
	{
		...
		{
			$query = mysqli_query($con, "SELECT name FROM ".$pixelpost_db_prefix."categories WHERE id='$cat_id' ORDER BY name");
			$images_category_or_date = mysqli_fetch_array($query);
			$images_category_or_date = pullout($images_category_or_date['name']??null);
		}
		else
		{
			$query = mysqli_query($con, "SELECT alt_name FROM ".$pixelpost_db_prefix."categories WHERE id='$cat_id' ORDER BY alt_name");
			$images_category_or_date = mysqli_fetch_array($query);
	...
	{
		if (isset($_GET['tag']))	$images_category_or_date = mysqli_real_escape_string($con, $_GET['tag']);
		else if (isset($_GET['archivedate']))	{
			$images_category_or_date = mysqli_real_escape_string($con, $_GET['monthname']);
...
$query = mysqli_query($con, "SELECT * FROM ".$pixelpost_db_prefix."categories ORDER BY name");
...
if(isset($PP_supp_lang)) $default_language_abr = strtolower($PP_supp_lang[$cfgrow['langfile']][0]);

//以下得出每个tag与pixelpost里面内容的最大匹配次数 (即每个tag的最大引用次数），然后再取得其中的最大值（LIMIT 1）

if(isset($language_abr) &&  $language_abr == $default_language_abr)	$queryr = "SELECT COUNT(*) AS max
		FROM {$pixelpost_db_prefix}pixelpost AS p, {$pixelpost_db_prefix}tags AS t
		WHERE p.id = t.img_id AND p.datetime<='$cdate'
		GROUP BY t.tag
		ORDER BY max DESC
		LIMIT 1";
elseif (isset($cdate))	$queryr = "SELECT COUNT(*) AS max
		FROM {$pixelpost_db_prefix}pixelpost AS p, {$pixelpost_db_prefix}tags AS t
		WHERE p.id = t.img_id AND p.datetime<='$cdate'
		GROUP BY alt_tag
		ORDER BY max DESC
...
$tag_max = mysqli_query($con, $queryr);
$tag_max = mysqli_fetch_row($tag_max);
$tag_max = $tag_max[0];


if (isset($language_abr) && $language_abr == $default_language_abr)	$queryr = "SELECT tag, COUNT(*) as cnt
		FROM {$pixelpost_db_prefix}pixelpost AS p, {$pixelpost_db_prefix}tags AS t
		WHERE p.id = t.img_id AND p.datetime<='$cdate'
		GROUP BY tag
		ORDER BY tag";
elseif (isset($cdate))	$queryr = "SELECT alt_tag, COUNT(*) as cnt
		FROM {$pixelpost_db_prefix}pixelpost AS p, {$pixelpost_db_prefix}tags AS t
		WHERE p.id = t.img_id AND p.datetime<='$cdate'
		GROUP BY alt_tag
		ORDER BY alt_tag";
else	$queryr = "SELECT alt_tag, COUNT(*) as cnt
		FROM {$pixelpost_db_prefix}pixelpost AS p, {$pixelpost_db_prefix}tags AS t
		WHERE p.id = t.img_id 
		GROUP BY alt_tag
		ORDER BY alt_tag";
...
$tags = mysqli_query($con, $queryr);

while(null!=$tags && isset($cdate) && list($tag, $cnt)  = mysqli_fetch_array($tags))
{
	$rank = round($cnt/$tag_max,1);
	$tags_output .= '<a href="index.php?x=browse&amp;tag='.$tag.'" class="tags'.substr($rank,0,1).substr($rank,2,1).'">'.$tag.'&nbsp;('.$cnt.')</a> ';
	$tags_paged_output .= '<a href="index.php?x=browse&amp;tag='.$tag.'&amp;pagenum=1" class="tags'.substr($rank,0,1).substr($rank,2,1).'">'.$tag.'&nbsp;('.$cnt.')</a> ';
	$tags_output_no_num .= '<a href="index.php?x=browse&amp;tag='.$tag.'" class="tags'.substr($rank,0,1).substr($rank,2,1).'">'.$tag.'</a> ';
	$tags_paged_output_no_num .= '<a href="index.php?x=browse&amp;tag='.$tag.'&amp;pagenum=1" class="tags'.substr($rank,0,1).substr($rank,2,1).'">'.$tag.'</a> ';
	$rank_reversed = $rank;

	if (substr($rank,0,1)=='1')
	{
		$rank_reversed=0; //give the most important tag the lessest weight
	}
	else
	{
  ...
  //print($tag.'-'.$rank_reversed[2].'<br>');
	...
	$tags_output_reversed .= '<a href="index.php?x=browse&amp;tag='.$tag.'" class="tags'.substr($rank_reversed,0,1).substr($rank_reversed,2,1).'">'.$tag.'&nbsp;('.$cnt.')</a> ';
	$tags_paged_output_reversed .= '<a href="index.php?x=browse&amp;tag='.$tag.'&amp;pagenum=1" class="tags'.substr($rank_reversed,0,1).substr($rank_reversed,2,1).'">'.$tag.'&nbsp;('.$cnt.')</a> ';
	$tags_output_reversed_no_num .= '<a href="index.php?x=browse&amp;tag='.$tag.'" class="tags'.substr($rank_reversed,0,1).substr($rank_reversed,2,1).'">'.$tag.'</a> ';
	$tags_paged_output_reversed_no_num .= '<a href="index.php?x=browse&amp;tag='.$tag.'&amp;pagenum=1" class="tags'.substr($rank_reversed,0,1).substr($rank_reversed,2,1).'">'.$tag.'</a> ';
}
...
if (!empty($tags)){
	if (isset(mysqli_fetch_row($tags)[0]))
	{
		while(list($tag)  = mysqli_fetch_array($tags))
		{
...
$tpl = str_replace("<TAG_LINKS_AS_LIST>",$tags_output,$tpl??null); //thumbnails in this page
