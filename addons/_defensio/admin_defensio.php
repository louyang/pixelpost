...
$addon_version = '1.4';
$result = mysqli_query($con, "SELECT `status` FROM `{$pixelpost_db_prefix}addons` WHERE `addon_name` LIKE '%akismet%' and `type`='front'") or die(mysql_error());
$akismet = mysqli_fetch_array($result);
...
//		$query = "SELECT `start_datetime` FROM `{$pixelpost_db_prefix}spamlog` LIMIT 1";
//    if (!mysqli_query($con, ($query)) {update_spamlog_table_for_defensio();}
//}

$result = mysqli_query($con, "SELECT * FROM `{$pixelpost_db_prefix}defensio` LIMIT 1");
$defensio_conf = mysqli_fetch_array($result);
...
$tpl = preg_replace("/<DEFENSIO_WIDGET>/", $defensio_widget, $tpl);
...
function get_defensio_links()
{
    ...
    {
        // get obvious spam
        $result = mysqli_query($con, "select count(*) as count from `" . $pixelpost_db_prefix . "comments` WHERE `publish`='dfn' AND `spaminess` < '" . $defensio_conf['threshold'] . "'") or die(mysql_error());
        $defensio_count_smaller_threshold = mysqli_fetch_array($result);
        $result = mysqli_query($con, "select count(*) as count from `" . $pixelpost_db_prefix . "comments` WHERE `publish`='dfn' AND `spaminess` >= '" . $defensio_conf['threshold'] . "'") or die(mysql_error());
        $defensio_count_larger_threshold = mysqli_fetch_array($result);
        echo "<br /><br />";
...
function defensio_settings()
{
    ...
    // get the settings
    $result = mysqli_query($con, "SELECT * FROM `{$pixelpost_db_prefix}defensio` LIMIT 1") or die(mysql_error());
    $defensio_conf = mysqli_fetch_array($result);
    $v = array();
...
function defensio_get_stats($defensio_conf)
{
    ...
    if (isset($ar['defensio-result']))
    {
        ...
        $defensio_stats_updated_at = time();
        mysqli_query($con, "UPDATE " . $pixelpost_db_prefix . "defensio SET defensio_stats='" . $defensio_stats . "', defensio_stats_updated_at='" . $defensio_stats_updated_at . "'");
    	...
    	$one_hour = 60 * 60;
    	$defensio_stats_updated_at = time() - $one_hour;
        mysqli_query($con, "UPDATE " . $pixelpost_db_prefix . "defensio SET defensio_stats_updated_at='" . $defensio_stats_updated_at . "'");
        return false;
}
...
function defensio_show_stats($defensio_conf)
{
    ...
    $two_hours = 60 * 60 * 2;
    if (($last_updated == null) or (time() - $last_updated > $two_hours))
    {
