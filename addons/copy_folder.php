...
// SVN file version:
// $Id: copy_folder.php 513 2008-01-16 18:43:22Z schonhose $
...
if( isset($_POST['folder_path']) && isset($_POST['copyfolder']))
{
	...
	foreach($files as $k => $file)
	{
		...
		if(copy($file,$newpath))
		{
			...
			mysqli_query($con, "update ".$pixelpost_db_prefix."pixelpost set exif_info='$exif_info_db' where id='$theid'");
			if (mysql_error())
			{
...
function category_list_as_table_noecho()
{
 ...
 // begin of category-list as a table
	...
	$catcounter = 0;
  $query = mysqli_query($con, "select id, name from ".$pixelpost_db_prefix."categories  order by name");
