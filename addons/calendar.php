...
if ( $geos_cat == 1){
	...
	if ( $_GET['showimage'] !="" && $_GET['category'] ==""){
		...
		$query22 = mysqli_query($con, "select cat_id from ".$pixelpost_db_prefix."catassoc where image_id='$showimage'");
		while(list($cat_id) = mysqli_fetch_row($query22)) {
		$get_cat_id = $cat_id;
...
$curr_month = $_GET['curr_month'] ?? null;
$curr_year = $_GET['curr_year'] ?? null;
if(!isset($curr_year)) { $curr_year = date("Y"); }
if(!isset($curr_month)) { $curr_month = date("n"); }
if(!isset($curr_day)) { $curr_day = date("j"); }
$showimage = $_GET['showimage'] ?? null;
if(!isset($_GET['showimage'])){ if(isset($image_id)) $showimage = $image_id; }


if (isset($_GET['showimage']) && !isset($_GET['curr_month'])){
       $showimage = (int)$_GET['showimage'];
	

	$query2 = mysqli_query($con, "select * from ".$pixelpost_db_prefix."pixelpost where id='$showimage'");
	while(list($img_id,$img_datetime) = mysqli_fetch_row($query2)) {
...
if ($next_month <= 9){ $next_month = "0$next_month";}
	...
	if ( $geos_cat == 1){
	$query2 = mysqli_query($con, "SELECT a.* FROM ".$pixelpost_db_prefix."pixelpost a, ".$pixelpost_db_prefix."catassoc b WHERE b.cat_id = '" . $_GET["category"] . "' AND a.id = b.image_id AND (a.datetime like '$prev_browsing_month_day%') ORDER BY a.datetime desc limit 1");
	}
	if ( ($geos_cat == 1 && $_GET["category"] == "") || $geos_cat != 1) {
	$query2 = mysqli_query($con, "select * from ".$pixelpost_db_prefix."pixelpost where datetime like '$prev_browsing_month_day%' order by datetime desc limit 1");
	}
	...
	if ( $geos_cat == 1){
	$query3 = mysqli_query($con, "SELECT a.* FROM ".$pixelpost_db_prefix."pixelpost a, ".$pixelpost_db_prefix."catassoc b WHERE b.cat_id = '" . $_GET["category"] . "' AND a.id = b.image_id AND (a.datetime like '$next_browsing_month_day%') ORDER BY a.datetime desc limit 1");
	}
	if ( ($geos_cat == 1 && $_GET["category"] == "") || $geos_cat != 1) {
	$query3 = mysqli_query($con, "select * from ".$pixelpost_db_prefix."pixelpost where datetime like '$next_browsing_month_day%' order by datetime desc limit 1");
	}

	while(list($img_id,$img_datetime) = mysqli_fetch_row($query3)) {
...
while($day_count <= $total_days[$curr_month]) {
	...
	if ( $geos_cat == 1){
	$query = mysqli_query($con, "SELECT a.* FROM ".$pixelpost_db_prefix."pixelpost a, ".$pixelpost_db_prefix."catassoc b WHERE b.cat_id = '" . $_GET["category"] . "' AND a.id = b.image_id AND (a.datetime like '$image_search%') and (a.datetime <= '$cdate') ORDER BY a.datetime");
	}
	if ( ($geos_cat == 1 && $_GET["category"] == "") || $geos_cat != 1) {
	$query = mysqli_query($con, "select * from ".$pixelpost_db_prefix."pixelpost where (datetime like '$image_search%') and (datetime<='$cdate') order by datetime");
	}

	while(list($img_id, $img_datetime, $img_headline, $img_body, $img_image) = mysqli_fetch_row($query)) {
		$dayimage = "true";
...
while($day_count <= $total_days[$curr_month]) {
	...
	if ( ($geos_cat == 1 && $_GET["category"] == "") || $geos_cat != 1) {
		if(isset($cdate))	$query = mysqli_query($con, "select * from ".$pixelpost_db_prefix."pixelpost where (datetime like '$image_search%') and (datetime<='$cdate') order by datetime");
		else $query = mysqli_query($con, "select * from ".$pixelpost_db_prefix."pixelpost where (datetime like '$image_search%') order by datetime");
	}
	...
	";
	$tpl = preg_replace("/<SITE_CALENDAR>/",$cal_hz,$tpl??null);
	}
