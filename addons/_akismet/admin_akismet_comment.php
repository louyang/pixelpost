...
// Echo the links
	...
	$query = mysqli_query($con, "select count(*) as count from ".$pixelpost_db_prefix."comments  WHERE publish='spm' ");
	$akismet_count = mysqli_fetch_array($query);
...
if ($fieldexists == 0) {
  $result = mysqli_query($con, "ALTER TABLE $table ADD `akismet_key` VARCHAR( 50 ) DEFAULT '' NOT NULL	");
}
// if the akismet_keep field does not exist, create it
if(!mysqli_query($con, "SELECT akismet_keep from ".$pixelpost_db_prefix."config")) mysqli_query($con, "ALTER TABLE ".$pixelpost_db_prefix."config ADD `akismet_keep` INT DEFAULT '7' NOT NULL ") or die ('<span style="color:red"><b>Error: '. mysql_error());
...
function akismet_settings(){
	...
	if ((isset($_GET['optaction'])) && ($_GET['optaction'] == "updateantispam")) {
	  ...
	  if ($update = mysqli_query($con, $query )) $addon_description .= "<font color='#006600'>&quot;Days to keep&quot; succesfully saved.</font>";
