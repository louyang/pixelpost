...
{
	global $cfgrow;
	if(!isset($_SESSION["pixelpost_admin"])) die ("Try another day!!");

	// Check to see if the ban table exists, if not, create it
	$query = "SELECT id FROM {$pixelpost_db_prefix}stats LIMIT 1";
	if( !mysqli_query($con, $query))	create_stat_table($pixelpost_db_prefix);
	else
	{
		$query = "select count(*) as count from ".$pixelpost_db_prefix."visitors";
		$countall = mysqli_query($con, $query);
		$countall = mysqli_fetch_array($countall);
		...
		while(null!=$mostvisitors && list($count,$referer,$ip) = mysqli_fetch_row($mostvisitors))
		{
    ...
    while(list($month) = mysqli_fetch_row($query))
    {
			if ($_GET['advncstat']??null!='domonth' && $month==$crrntmonth)	continue;
...
{
	$visitorz = mysqli_query($con, "SELECT sum(visitors) as count FROM ".$pixelpost_db_prefix."stats");
	$visitorz = mysqli_fetch_array($visitorz);
	$pixelpost_visitors  += $visitorz['count'];
	$str = $pixelpost_visitors .'';
	$tpl = preg_replace("/<ADVNCD_SITE_VISITORNUMBER>/",$str,$tpl??null);
}
...
function create_stat_table($pixelpost_db_prefix)
{
	...
	if (!mysqli_query($con,  $query))	$somestates .= "<br/>Error occured in creating table: ".mysql_error();
	...
	if(!mysqli_query($con, $insert_query))
	{
		...
		mysqli_query($con, " drop TABLE '{$pixelpost_db_prefix}stats' ");
	}
}
...
function build_insertquery($pixelpost_db_prefix,$where,$thismonth)
{
	...
	$mostips = mysqli_query($con, $query);
	$m = 0;

	while(list($count,$ip) = mysqli_fetch_row($mostips))
	{
...
function delete_month_visitors ($month)
{
	...
	if(!mysqli_query($con, $query))	$message = mysql_error();
...
function stat_update_month($pixelpost_db_prefix,$thismonth)
{
	...
	else
	{
		$where = " where DATE_FORMAT(datetime, '%Y-%m') <= '".$thismonth."'";
		$insert_query = build_insertquery($pixelpost_db_prefix,$where,$thismonth);
		if(mysqli_query($con, $insert_query))	$message = "ok";//Month ".$thismonth." is added!";
...
function add_current_month($pixelpost_db_prefix)
{
	...
	if (mysqli_query($con, $query))
	{
		...
		if(mysqli_query($con, $updatequery))	$message = "Month ".$thismonth." is updated!";
	...
	else
	{
		...
		if(mysqli_query($con, $insert_query))	$message = "Month ".$thismonth." is added!";
		else	$message = "Inserting Month ".$thismonth." failed!";
	}
	return $message ;
}
