...
if(isset($_GET['x']) && $_GET['x'] == "rss" && !isset($_GET['tag']))
{
	if(($cfgrow['feed_discovery'] == 'RA' || $cfgrow['feed_discovery'] == 'R') || ($cfgrow['feed_discovery'] == 'E' && $cfgrow['feed_external_type'] == 'ER'))
	{
		...
		$query = mysqli_query($con, "SELECT `id`,`datetime`,`headline`,`body`,`image`,`alt_headline`,`alt_body` FROM `".$pixelpost_db_prefix."pixelpost` WHERE (`datetime`<='$cdate') ORDER BY `datetime` DESC LIMIT ".$feeditems);

		while(list($id,$datetime,$headline,$body,$image,$alt_headline,$alt_body) = mysqli_fetch_row($query))
		{
...
if(isset($_GET['x']) && $_GET['x'] == "comment_rss")
{
	...
	if(($cfgrow['allow_comment_feed'] == 'Y') && ($cfgrow['feed_discovery'] == 'RA' || $cfgrow['feed_discovery'] == 'R') || ($cfgrow['feed_discovery'] == 'E' && $cfgrow['feed_external_type'] == 'ER'))
	{
		...
		while(list($comment_id,$comment_datetime,$comment_message,$comment_name,$comment_url,$parent_id,$parent_headline,$parent_image,$alt_headline) = mysqli_fetch_row($query))
		{
...
if(isset($_GET['x']) && $_GET['x'] == "comment_atom")
{
	...
	if(($cfgrow['allow_comment_feed'] == 'Y') && ($cfgrow['feed_discovery'] == 'RA' || $cfgrow['feed_discovery'] == 'A') || ($cfgrow['feed_discovery'] == 'E' && $cfgrow['feed_external_type'] == 'EA'))
	{
		...
		while( list($comment_id, $comment_datetime, $comment_message, $comment_name, $comment_url, $parent_id, $parent_headline, $parent_image, $alt_headline, $alt_body) = mysqli_fetch_row($query))
		{
			if ($language_abr == $default_language_abr)
			{
				$parent_headline = pullout($parent_headline);
...
$tpl = preg_replace("/<ATOM_AUTODETECT_LINK>/",$atom_auto,$tpl);
$tpl = preg_replace("/<SITE_ATOM_LINK>/","<a href='./index.php?x=atom'>ATOM feed</a>",$tpl);

if (isset($_GET['tag']) && $_GET['tag'] != '') $tpl = preg_replace("/<TAG_ATOM_LINK>/","<a href='./index.php?x=atom&amp;tag=".$_GET['tag']."'>ATOM feed for &quot;".$_GET['tag']."&quot;</a>",$tpl);
else $tpl = preg_replace("/<TAG_ATOM_LINK>/","",$tpl);
...
$tpl = preg_replace("/<COMMENT_ATOM_AUTODETECT_LINK>/",$comment_atom_auto,$tpl);
$tpl = preg_replace("/<SITE_COMMENT_ATOM_LINK>/","<a href='./index.php?x=comment_atom'>ATOM Comment feed</a>",$tpl);

// RSS Tags
$rss_auto = "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".$feed_title." - RSS Feed\" href=\"".$cfgrow["siteurl"]."index.php?x=rss\" />";
$tpl = preg_replace("/<RSS_AUTODETECT_LINK>/",$rss_auto,$tpl);
$tpl = preg_replace("/<SITE_RSS_LINK>/","<a href='./index.php?x=rss'>RSS 2.0</a>",$tpl);

if (isset($_GET['tag']) && $_GET['tag'] != '') $tpl = preg_replace("/<TAG_RSS_LINK>/","<a href='./index.php?x=rss&amp;tag=".$_GET['tag']."'>RSS 2.0 for &quot;".$_GET['tag']."&quot;</a>",$tpl);
else $tpl = preg_replace("/<TAG_RSS_LINK>/","",$tpl);
...
$tpl = preg_replace("/<COMMENT_RSS_AUTODETECT_LINK>/",$comment_rss_auto,$tpl);
$tpl = preg_replace("/<SITE_COMMENT_RSS_LINK>/","<a href='./index.php?x=comment_rss'>Comment RSS</a>",$tpl);
...
$tpl = preg_replace("/<FEED_AUTO_DISCOVERY>/",$feed_auto_discovery,$tpl);
?>