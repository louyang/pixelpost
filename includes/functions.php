...
function print_comments($imageid)
{
	...
	$comment_count = 0;
	$image_comments = ""; // comments stored in this string, use to have <ul>, deleted for alignment
...
{
	global $admin_lang_pp_up_error_0, $admin_lang_pp_up_error_1, $admin_lang_pp_up_error_2, $admin_lang_pp_up_error_3, $admin_lang_pp_up_error_4, $admin_lang_pp_up_error_6,$admin_lang_pp_up_error_7;
...
///////////////////////////////////////////////////////////////////////////////////////////////  
    ...
    if ($radius == 0) { 
        return $img; imagedestroy($img); exit;        } 
...
function sql_array($str)
{
	...
	$query = mysqli_query($con, $str) or die( mysqli_error());
...
function &reduceExif($exifvalue)
{
	...
	if(isset($vals))
	{
		...
		{
			$exposure = round(floatval($vals[0])/floatval($vals[1]),2);
			if ( $exposure < 1) $exposure = '1/'.round(floatval($vals[1])/floatval($vals[0]),0);
		}
	}
	else	$exposure = "";
 ...
 // begin of category-list as a table
	...
	while(list($id,$name) = mysqli_fetch_row($query))
	{
		...
		while(list($id,$name,$alt_name,$image_id) = mysqli_fetch_row($query))
		{
			...
			$id = pullout($id);
			if (isset($catcounter)) $catcounter++;
			else $catcounter=1;
			$inarow = 4;
			...
			{
				if ($firstid==$id && isset($_GET['view']) && $_GET['view']!='images') // if it is the first defualt category in the new_image page
	...
	{// addons table does exist
		...
		while (list($id,$addon_name)= mysqli_fetch_row($query))
		{
			if (!file_exists($dir.$addon_name.".php"))
			{
				...
				$resquerydel = mysqli_query($con, $querydel);
...
function create_front_addon_array()
{
	...
	while (list($id,$filename,$status,$addon_type)= mysqli_fetch_row($result))
	{
		include_once($addon_dir.$filename.".php");
		echo $addon_dir.$filename.".php";
	} // end while
}
...
function create_banlist()
{
	...
	if (!banlist_exist()){
	  ...
	  mysqli_query($con,  $query);
	  if (mysqli_error($con))
	  	$result = "$admin_lang_spam_err_1".mysqli_error($con);
	  else
...
{
	global $pixelpost_db_prefix, $phpver, $con;
	if( isset( $_POST['banlistupdate']))
	{
		...
		if (isset( $_POST['moderation_list']))
		{
			...
			if (version_compare($phpver,"4.3.0")=="-1")	$banlist = mysqli_escape_string($con, $banlist);
			else	$banlist = mysqli_real_escape_string($con, $banlist);
		...
		if (!isset($result))	$result = "\$admin_lang_spam_upd";   //\$转义
...
function create_htaccess_banlist()
{
	...
	$to_htaccess = $denylist??null.$badreflist;
...
{
	global $pixelpost_db_prefix, $admin_lang_spam_err_6, $admin_lang_spam_com_upd, $con;
	//	if( isset( $_POST['banlistupdate'])){
	// moderation list
	//	if (isset( $_POST['moderation_list'])) {
	...
	if (isset($_GET['antispamaction']) && $_GET['antispamaction']=='moderation')
	{
		...
		if (mysqli_error($con))	$additional_msg = $admin_lang_spam_err_6.'&nbsp;'.mysqli_error($con).'<br />';
...
{
	global $pixelpost_db_prefix, $admin_lang_spam_com_del, $admin_lang_spam_err_7, $con;
	//	if( isset( $_POST['banlistupdate'])){
	// moderation list
	//	if (isset( $_POST['moderation_list'])) {
	...
	$where ='';
	//if moderation of past comments pressed
	if (isset($_GET['antispamaction']) && $_GET['antispamaction']=='deletecmnt')
	{
		...
		mysqli_query($con, $query);
		if (mysqli_error($con))	$additional_msg = $admin_lang_spam_err_7.'&nbsp;'.mysqli_error($con).'<br />';
...
{
	global $pixelpost_db_prefix, $admin_lang_spam_err_8, $admin_lang_spam_visit_del, $con;
	//	if( isset( $_POST['banlistupdate'])){
	// moderation list
	//	if (isset( $_POST['moderation_list'])) {
	...
	$where ='';
	//if moderation of past comments pressed
	if (isset($_GET['antispamaction']) && $_GET['antispamaction']=='deleterefs')
	{
...
//echo $sql_tag;				//exit('stop here');  备注,可能因为新版SQL必须全部字段写入才能成功. 在Mysql字段将alt_tag设置为了Null
		mysqli_query($con, $sql_tag);
	}
}
...
function list_tags_edit($id,$alt="")
{
	...
	$sql_tag = "SELECT ".$alt."tag FROM " . $pixelpost_db_prefix . "tags WHERE img_id = " . $id . " AND ".$alt."tag NOT LIKE '' ORDER BY tag_id ASC";
	$query = mysqli_query($con, $sql_tag);
