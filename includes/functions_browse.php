...
if(isset($_GET['x']) &&$_GET['x'] == "browse")
{
	...
	if(is_numeric($pp_cat) && $pp_cat != "")
	{
		...
		$query = mysqli_query($con, "SELECT 1,t2.id,{$headline_selection},image,datetime
	...
	elseif(isset($_GET['archivedate']) && preg_match("/^[0-9]{4}-[0-9]{2}$/i", $_GET['archivedate']))
	{
		$archivedate = preg_replace('/[^0-9\-]/', '', $_GET['archivedate']);
		$where = "AND (DATE_FORMAT(datetime, '%Y-%m')='$archivedate')"; //DATE_FORMAT(foo, '%Y-%m-%d')
		$query = mysqli_query($con, "SELECT 1,id,{$headline_selection},image, datetime FROM ".$pixelpost_db_prefix."pixelpost WHERE (datetime<='$cdate') $where ORDER BY ".$cfgrow['display_sort_by']." ".$display_order);
		$lookingfor = 1;
	}
	...
	{
		$lookingfor = 1;
 		$query = mysqli_query($con, "SELECT 1,id,{$headline_selection},image,datetime FROM ".$pixelpost_db_prefix."pixelpost WHERE (datetime<='$cdate') ORDER BY ".$cfgrow['display_sort_by']." ".$display_order);
	}

	while(list($count,$id,$title,$name,$datetime) = mysqli_fetch_row($query))
	{
...
$query = mysqli_query($con, "SELECT * FROM ".$pixelpost_db_prefix."categories ORDER BY ".$browse_order_by);
