...
function serialize_exif ($uploaded_file)
{
  ...
  foreach ($flat_exif_tmp as $key=>$value) {
	...
	// we need to escape the string before saving it to the db
 	$exif_info = mysqli_real_escape_string($con, $exif_info);
	return $exif_info;
}
...
function replace_exif_tags ($language_full, $image_exif, $tpl)
{
    ...
    {
    	$exposure = "$exposure";
    	$tpl = preg_replace("/<EXIF_EXPOSURE_TIME>/",$exposure,$tpl);
    }
    else
    {
    	$exposure = "$empty_exif";
    ...
    $tpl = preg_replace("/<LANG_EXPOSURE_TIME>/",$langexposure,$tpl);


    if(isset($aperture)&&$aperture != "")
    {
    	$tpl = preg_replace("/<EXIF_APERTURE>/",$aperture,$tpl);
    }
    else
    {
    	$aperture = "$empty_exif";
    ...
    $tpl = preg_replace("/<LANG_APERTURE>/",$langaperture,$tpl);

    if(isset($capture_date)&&$capture_date != "")
    {
    	$tpl = preg_replace("/<EXIF_CAPTURE_DATE>/",$capture_date,$tpl);
    }
    else
    {
    	$capture_date = "$empty_exif";
    ...
    $tpl = preg_replace("/<LANG_CAPTURE_DATE>/",$langcapture_date,$tpl);

    if(isset($focal)&&$focal != "")
    {
    	$tpl = preg_replace("/<EXIF_FOCAL_LENGTH>/",$focal,$tpl);
    }
    else
    {
    	$focal = "$empty_exif";
    ...
    $tpl = preg_replace("/<LANG_FOCAL_LENGTH>/",$langfocal,$tpl);

    if(isset($info_camera_manu)&&$info_camera_manu != "")
    {
    	$tpl = preg_replace("/<EXIF_CAMERA_MAKE>/",$info_camera_manu,$tpl);
    }
    else
    {
    	$info_camera_manu = "$empty_exif";
    ...
    $tpl = preg_replace("/<LANG_CAMERA_MAKE>/",$langcamera_manu,$tpl);

    if(isset($info_camera_model)&&$info_camera_model != "")
    {
    	$tpl = preg_replace("/<EXIF_CAMERA_MODEL>/",$info_camera_model,$tpl);
    }
    else
    {
    	$info_camera_model = "$empty_exif";
    ...
    $tpl = preg_replace("/<LANG_CAMERA_MODEL>/",$langcamera_model,$tpl);

    if(isset($iso)&&$iso != "")
    {
    	$tpl = preg_replace("/<EXIF_ISO>/",$iso,$tpl);
    	$iso = "$iso";
    }
    else
    {
    ...
    $tpl = preg_replace("/<LANG_ISO>/",$langiso,$tpl);

   	$flash = "$flash";
   	$tpl = preg_replace("/<EXIF_FLASH>/",$flash,$tpl);
  }
	return $tpl;
}
...
{
	$tpl = preg_replace("/<EXIF_EXPOSURE_TIME>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_EXPOSURE_TIME>",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_APERTURE>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_APERTURE>/",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_CAPTURE_DATE>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_CAPTURE_DATE>/",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_FOCAL_LENGTH>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_FOCAL_LENGTH>/",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_CAMERA_MAKE>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_CAMERA_MAKE>/",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_CAMERA_MODEL>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_CAMERA_MODEL>/",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_ISO>/",NULL,$tpl);
	$tpl = preg_replace("/<LANG_ISO>/",NULL,$tpl);
	$tpl = preg_replace("/<EXIF_FLASH>/",NULL,$tpl);
	return $tpl;
}
