...
if (isset($_GET['optaction']) && $_GET['optaction']=='updateall')
{
	if(!isset($_GET['optionsview']) OR $_GET['optionsview']=='general')
	{
		//case "updatetitle":
		//case "updatesubtitle":
		//case "updateurl":
...
if (isset($_GET['optaction'])) switch ($_GET['optaction'])
{
...
if(isset($_GET['optaction']))
{
	if ($lang_error==0){
		...
		if (isset($_POST['token_time']) && $_POST['token_time'] < 1 && $_GET['optionsview'] == 'antispam') {
...
else $submenucssclass = 'selectedsubmenu';
...
if (!isset($_GET['optionsview']) or $_GET['optionsview']=='general')
{
        ...
        if($handle = opendir($dir)) { 
			while (false !== ($file = readdir($handle))) {
				...
				if(is_file('../language/'.$file) && ($file != "index.html") && $admin_pre == "admin-") {
					$file = preg_replace("/admin-lang-/","",$file);
					$file = preg_replace("/.php/","",$file);
    ...
    <div class='content'>$admin_lang_optn_cmnt_mod_txt
		...
		<option value='$optnval'>$optnecho</option>
				...
				if (!isset($admin_lang_optn_comment_moderation)) $tmp = "";
				else $tmp = $admin_lang_optn_comment_moderation;
				echo "
	...
	echo "
		...
		<input type='text' name='thumbheight' value='".$cfgrow['thumbheight']."' /><br>
