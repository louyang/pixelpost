...
if(isset($_GET['view']) && $_GET['view'] == "info")
{
  ...
  if($cfgquery = mysqli_query($con, "select * from ".$pixelpost_db_prefix."config"))	$cfgrow = mysqli_fetch_assoc($cfgquery);
  eval_addon_admin_workspace_menu("info","info");
  // end add a workspace	

if (!isset($_GET['infoview']) OR $_GET['infoview']=='general')
{
	...
	if(function_exists('gd_info'))
	{
		...
		else if (isset($gd_info1["JPG Support"]) && $gd_info1["JPG Support"]) $gd_info .= " $admin_lang_info_gd_jpg";
	}		// func exist

  $version[1] = number_format($version[1],1);
  ...
  echo "<div class='jcaption'>$admin_lang_pixelpostinfo</div>
    <div class='content'>
    $admin_lang_pp_currversion $version[1] updated on $version[0]<br>
    $admin_lang_pp_forum: <a href='http://forum.pixelpost.org/'>forum.pixelpost.org</a> <br>
    Link to the Github page: <a href='https://github.com/louyang/pixelpost/'>github.com/louyang/pixelpost</a>
    </div>
    <p />
...
// exif infomation: exifer
	...
	<i><b>$admin_lang_pp_imagepath  ";
	 ...
	 $guess_path = preg_replace("/admin/i","",$guess_path);
	...
	<b>$admin_lang_pp_thumbnailpath_conf</b> ".$cfgrow['thumbnailpath']."<p />";
	 ...
	 $work_path = preg_replace("/images/i","",$cfgrow['imagepath']);
...
echo "
    ...
    $referer = "";
    $query = mysqli_query($con, "select distinct referer from ".$pixelpost_db_prefix."visitors where (referer!='') AND (datetime>'$from_date')");
    ...
    $ref_biglist = explode("!",$ref_biglist);
    rsort($ref_biglist,SORT_NUMERIC);
    if (isset($ref_biglist) && $ref_biglist[0]<>"")
    {
