...
if (isset($_GET["_SESSION"]["pixelpost_admin"]))
{
...
// view=images
if(isset($_GET['view']) && $_GET['view'] == "images")
{
	if(isset($_GET['action']) && $_GET['action'] == "masspublish")
	{
	...
	if(isset($_GET['action']) && $_GET['action'] == "massdelete")
	{
 		$idz= $_POST['moderate_image_boxes'];
	...
	if(isset($_GET['action']) && $_GET['action'] == "masscatedit")
	{
		$command = $_GET['cmd'];
	...
	if(isset($_GET['masstagopt']))
	{
		...
		else
		{
			...
			if(count($idz) > 0)	$query  = @mysqli_query($con, $query);
		}
	}

  // x=update
  if(isset($_GET['x']) && $_GET['x'] == "update")
  {
		$headline = clean($_POST['headline']);
		$body = clean($_POST['body']);
		$alt_headline = clean($_POST['alt_headline']??null);
		$alt_body = clean($_POST['alt_body']??null);
		...
		$result = mysqli_query($con, $query) ||("Error: ".mysqli_error($con));
...
//------------
		if (isset($_POST['category']))
		{
			...
			$query_st = "INSERT INTO ".$pixelpost_db_prefix."catassoc (id,cat_id,image_id) VALUES ".implode(",", $query_val).";";
			$result = mysqli_query($con, $query_st) || die("Error: ".mysqli_error($con));
		}
		...
		$result = mysqli_query($con, $query) ||("Error: ".mysqli_error($con).$admin_lang_imgedit_db_error);

		if (!isset($file_reupload_str)) $file_reupload_str="";
  ...
  if($_GET['id'] == "")
  {
	  ...
	  // calculate the number of pages
		...
		while(list($id,$name) = mysqli_fetch_row($query)) {
			if(isset($selectfcat)) $selectfcats .= "<option value='$id'".($id==$selectfcat?' selected=\"selected\"':'').">".pullout($name)."</option>\n";
		}
		// the query for the select by tag statement
		$selectftags = "<option value='' selected=\"selected\">All</option>\n";
		$query = mysqli_query($con, "select distinct tag from ".$pixelpost_db_prefix."tags WHERE tag NOT LIKE '' order by tag");
		while(list($tag) = mysqli_fetch_row($query)) {
			if(isset($selectftag)) $selectftags .= "<option value='$tag'".($tag==$selectftag?' selected=\"selected\"':'').">".$tag."</option>\n";
		}
		...
		if ($cfgrow['altlangfile'] != 'Off') {
			...
			$query = mysqli_query($con, "select distinct alt_tag from ".$pixelpost_db_prefix."tags WHERE alt_tag NOT LIKE '' order by alt_tag");
		...
		$query = mysqli_query($con, "select distinct DATE_FORMAT(datetime, '%Y-%m') as fmonth from ".$pixelpost_db_prefix."pixelpost order by datetime desc");
		while(null!=$query && list($fmonth) = mysqli_fetch_row($query)) {
			if(isset($selectfmon)) $selectfmons .= "<option value='$fmonth'".($fmonth==$selectfmon?' selected=\"selected\"':'').">".$fmonth."</option>\n";
		}
    ...
    if (isset($selectfcat)) {
		...
		while(list($id,$datetime,$headline,$body,$image,$category, $alt_headline) = mysqli_fetch_row($images))
		{
...
# 		$headline = htmlentities($headline);
			...
			$fs*=0.001;
 			...
 			$mib = [];
 			if(isset($_POST['moderate_image_boxes'])) $mib = $_POST['moderate_image_boxes'];   //∑¿÷πø’÷µ¥ÌŒÛ
			...
			echo "<li><a href=\"../index.php?showimage=$id\"><img src=\"".$cfgrow['thumbnailpath']."thumb_$image\" align=\"left\" hspace=\"3\" alt=\"Click to go to image\" /></a>
				<input type=\"checkbox\" class=\"images-checkbox\" name=\"moderate_image_boxes[]\" value=\"$id\" ".(in_array($id, $mib)?' checked':'')."/>
			...
			echo "<strong>$admin_lang_imgedit_category_plural &nbsp;</strong>";
					$category_list = mysqli_query($con, "SELECT t2.name FROM ".$pixelpost_db_prefix."catassoc t1 INNER JOIN ".$pixelpost_db_prefix."categories t2 ON t1.cat_id = t2.id WHERE t1.image_id = '$id' ORDER BY t2.name ");
  ...
  else
  {
    ...
    $query = mysqli_query($con, "SELECT `cat_id` FROM " . $pixelpost_db_prefix . "catassoc where image_id='$getid'");
    while(list($cat_id) = mysqli_fetch_row($query))
    {
      $category[] = $cat_id;
    }
    echo "
		...
		if (!isset($selectedclass)) $selectedclass = 'selectedsubmenu';
...
// edit image, list categories etc.
		...
		if (!isset($_GET['imagesview']) or $_GET['imagesview']=='edit') 
		{
