   ...
   // save new post
    ...
    if (isset($_POST['category']))
	  ...
	  $datetime =
		...
		if( $_POST['autodate'] == 1)
		{
			$query = mysqli_query($con, "select datetime + INTERVAL 1 DAY from ".$pixelpost_db_prefix."pixelpost order by datetime desc limit 1");
	  ...
	  // insert post in mysql
		...
		if($status == "ok")
		{
			$query = "INSERT INTO ".$pixelpost_db_prefix."pixelpost (datetime,headline,body,image,category,alt_headline,alt_body,comments,exif_info)
			VALUES('$datetime','$headline','$body','$image','$category','$alt_headline','$alt_body','$comments_settings','$exif_info_db')";
			$result = mysqli_query($con, $query) || die("Error: ".mysql_error().$admin_lang_ni_db_error);

	    $theid = mysqli_insert_id($con); //Gets the id of the last added image to use in the next "insert"
...
?>
  ...
  <?php echo $admin_lang_ni_select_cat;?>
	<?php
	category_list_as_table(clean($_POST['category']??null), $cfgrow);
 	$tz = $cfgrow["timezone"];
 	$cur_time = gmdate("Y-m-d H:i:s",time()+(3600 * $tz));
 	$cur_year = gmdate("Y",time()+(3600 * $tz));
	$_POST['autodate'] = clean($_POST['autodate']??null);
 ...
 ?>
    ...
    <div class='content'>
     <input type="radio" name="autodate" value="2" <?php if($selected_autodate[2]??null) echo 'checked="checked" ';?>id="postnow"/><label for="postnow"><?php echo $admin_lang_ni_post_now . ' (~'.$cur_time.')';?></label><br/>
     <input type="radio" name="autodate" value="1" <?php if($selected_autodate[1]??null) echo 'checked="checked" ';?>id="postdayaft"/><label for="postdayaft"><?php echo $admin_lang_ni_post_one_day_after;?></label><br/>
     <input type='radio' name='autodate' value='3' <?php if($selected_autodate[3]??null) echo 'checked="checked" ';?>id="exifdate"/><label for="exifdate"><?php echo $admin_lang_ni_post_exif_date;?></label><br/>
     <input type='radio' name='autodate' value='0' <?php if($selected_autodate[0]??null) echo 'checked="checked" ';?>id="specificdate"/><label for="specificdate"><?php echo $admin_lang_ni_post_spec_date;?></label><br/><br/>
 ...
 <tr><td>
  <select name='post_year' onchange="return ondatechange()" >
	...
	{
	  echo "<option";
	  if(isset($_POST['post_year']) && $_POST['post_year']==$lc)	echo " SELECTED";
	...
	while($lc <= 12)
	{
	  if($lc < 10) { $lc = "0$lc"; }
	  echo "<option";
	  if(isset($_POST['post_month']) && $_POST['post_month']==$lc)	echo " SELECTED";
	  echo " value='$lc'>$lc</option>";
	  $lc++;
	}
	?>
	...
	while($lc <= 31)
	{
	  if($lc < 10) { $lc = "0$lc"; }
	  echo "<option";
	  if(isset($_POST['post_day']) && $_POST['post_day']==$lc)	echo " SELECTED";
	  echo " value='$lc'>$lc</option>";
	  $lc++;
	}
	?>
	...
	while($lc < 24)
	{
	  if($lc < 10) { $lc = "0$lc"; }
	  echo "<option";
	  if(isset($_POST['post_hour']) && $_POST['post_hour']==$lc)	echo " SELECTED";
	  echo " value='$lc'>$lc</option>";
	  $lc++;
  }
	?>
	...
	while($lc <= 59)
	{
	  if($lc < 10) { $lc = "0$lc"; }
	  echo "<option";
	  if(isset($_POST['post_minute']) && $_POST['post_minute']==$lc)	echo " SELECTED";
	  echo " value='$lc'>$lc</option>";
	  $lc++;
	}
	?>
...
<?php
	if(isset($_GET['x']) && $_GET['x'] == "save" && $status == "ok")
	{
		...
		if(function_exists('gd_info'))
		{
			...
			if($gd_info != "")
			{
				...
				//$thumbnail2 = createthumbnail2($filnamn);
				eval_addon_admin_workspace_menu('thumb_created');

				// if crop is not '12c' use the oldfashioned crop
				if($cfgrow['crop']!='12c')
				{
					if($show_image_after_upload)	echo "<img src='".$cfgrow['imagepath'].$filnamn."', style='max-width:100%; height:auto;'/>";
