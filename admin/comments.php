...
if(isset($_GET['view']) && $_GET['view'] == "comments") {
 ...
 // edit a comment
	 ...
	 // Mass SPAM-delete comments
		if(isset($_GET['action']) && $_GET['action'] == "spamdelete" && isset($_POST['moderate_commnts_boxes'])) {
		$idz= $_POST['moderate_commnts_boxes'];
		...
		if(version_compare(phpversion(),"4.3.0")=="-1") {
			 $banlist = mysqli_escape_string($banlist);
		 } else {
			 $banlist = mysqli_real_escape_string($banlist);
		 }// end if
	 ...
	 // Mass publish comments
			if(isset($_GET['action']) && $_GET['action'] == "massunpublish") {
			$idz= $_POST['moderate_commnts_boxes'];
 ...
 if(isset($admin_lang_)) echo "<div id='caption'>$admin_lang_$admin_lang_comments</div>";
 
 // list of comments
    if(!isset($_GET['action'])) {
         $pagec = 0;
        if(!isset($_GET['page'])) { $page = "0"; } else { $page = intval($_GET['page']); }
    ...
    if($pixelpost_commentnumb > $_SESSION['numimg_pp'])
    {
				...
				if ($page >= $_SESSION['numimg_pp'])
	      {
	       ...
	       echo "\n".$image_page_Links."<br/>";
				...
				echo '<form method="post" action="'.$_SERVER['PHP_SELF'].'?'.pagenumber($_GET['page']??null).'" accept-charset="UTF-8">';
