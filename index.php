...
create_front_addon_array();   //载入设置为on的插件
...
// cleanup $_GET['x']
if(isset($_GET['x'])){$_GET['x'] = preg_replace('/[^a-z0-9_-]/i', '', $_GET['x']);}
...
if(!empty($language_full)) {
	...
	if (!preg_match("^[A-Za-z]+([0-9]+)?$^", $language_full) and !preg_match("^[A-Za-z]+[[:punct:]]+[A-Za-z]+)?$^", $language_full)) {
...
if(!isset($_GET['x'])) {
	...
	$tpl                      =   str_replace("<IMAGE1_WIDTH>",$local_width,$tpl);
	$tpl                      =   str_replace("<IMAGE1_HEIGHT>",$local_width,$tpl);
	...
	if(isset($next_row['image'])) $image_next_name =  $next_row['image'];
	else $image_next_name="";
	if(isset($next_row['id'])) $image_next_id   =  $next_row['id'];
	else $image_next_id="";

	if($language_abr == $default_language_abr) {

		if(isset($next_row['headline'])) $image_next_title = pullout($next_row['headline']);
		else $image_next_title="";
	}else{

		if(isset($next_row['alt_headline'])) $image_next_title = pullout($next_row['alt_headline']);
		else $image_next_title="";
	}

	if(isset($next_row['datetime'])) $image_next_datetime =  $next_row['datetime'];
	 ...
	 *
	 */
	if(isset($_GET['x']) && ($_GET['x'] == "") or (isset($_GET['popup']) && isset($_GET['parent_id']) && $_GET['popup'] == "comment")) {
...
if(!isset($_GET['x']) || isset($_GET['showimage'])){
	...
	if ($row['comments'] == 'F'){
		...
		$tpl = preg_replace("/<COMMENT_POPUP>/","<a href='index.php?showimage=$image_id' onclick=\"window.open('index.php?popup=comment&amp;showimage=$image_id','Comments','width=500,height=750,scrollbars=yes,resizable=yes');\">$lang_comment_popup</a>",$tpl);
	}
}
...
include_once('includes/addons_lib.php');  //载入设置为on的插件
